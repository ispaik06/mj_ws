...
The environment `ENV_ID=myhumanoid-v0` is registered automatically via the `envs` package, and you can run training/playback immediately using `scripts/train.py` and `scripts/play.py`.
...
│  │  └─ myhumanoid.py   # Environment class (MyhumanoidEnv)
│  ├─ assets/
...
The `ENV_ID=myhumanoid-v0` is registered with:
